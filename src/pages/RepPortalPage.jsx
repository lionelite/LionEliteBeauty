...
export default function RepPortalPage() {
  ...
  return <div style={{backgroundColor:'#0D0D0D',minHeight:'100vh',color:'#FAF7F2'}}>
    ...
    <section style={{paddingTop:'150px',paddingBottom:'110px'}}><div className="max-w-6xl mx-auto px-6">
      ...
      : dashboard.role === 'admin' ? <AdminDashboard dashboard={dashboard} money={money} signOut={signOut} copy={copy} copied={copied} />
...
function AdminDashboard({dashboard,money,signOut,copy,copied}) {
  ...
  async function createAffiliate(e){e.preventDefault();setSaving(true);setError('');setResult(null);try{const token=prompt('Enter the Beauty ADMIN_TOKEN to create this affiliate.');if(!token)throw new Error('Admin token is required');const res=await fetch('/api/rep',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'provision',token,name,email,code,discountPercent:Number(discount),commissionPercent:Number(commission)})});const data=await res.json();if(!res.ok)throw new Error(data.error||'Unable to create affiliate');setResult(data);setName('');setEmail('');setCode('')}catch(err){setError(err.message)}finally{setSaving(false)}}
