      ...
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <div className="flex flex-col">
          ...
          <span style={{ color: '#C9A96E', fontFamily: 'Helvetica Neue, Arial, sans-serif', letterSpacing: '0.2em' }}
            className="text-xs uppercase tracking-widest font-light">Beauty</span>
        </div>
        <div className="hidden md:flex items-center gap-8">
          {[{label:'Shop',href:'#products'},{label:'Science',href:'#science'},{label:'Results',href:'#results'},{label:'About',href:'#about'}].map(link => (
          ...
          <a href="#products"
            ...
            className="text-white text-xs uppercase tracking-wider px-6 py-3 hover:opacity-90 transition-opacity">Shop Now</a>
        </div>
      ...
      {menuOpen && (
        <div style={{ backgroundColor: '#FAFAF8', borderTop: '1px solid #E8DDD0' }} className="md:hidden px-6 py-6 flex flex-col gap-5">
          ...
          <a href="#products" onClick={() => setMenuOpen(false)}
            ...
            className="text-white text-xs uppercase tracking-wider px-6 py-3 text-center">Shop Now</a>
        </div>
      )}
    </nav>
  )
